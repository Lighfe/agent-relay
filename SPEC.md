...
Claiming atomically changes the task to `processing` and creates a delivery attempt with a new claim token. Concurrent workers cannot successfully claim the same task while that lease is active. The implementation uses `SELECT ... FOR UPDATE SKIP LOCKED` to atomically select the oldest available queued task and lock it in one operation, ensuring concurrent claims are safe without process-local locks or serialized transactions.
